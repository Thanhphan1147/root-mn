...
const HOWTO = {
  MC: {
    ...
    sections: [
      ...
      ["Daylight", "First craft with workshops. Then take 3 actions, plus 1 per bird card spent: March (up to two moves), Recruit (once per turn), Build (spend wood; score the build track), Overwork (spend a matching card for wood at a sawmill)."],
      ["Evening", "Draw 1 card plus the uncovered recruiter draw bonus (up to 3), then discard down to 5."],
      ["Rules", "You rule with warriors + buildings. Only you may place pieces in the Keep clearing. Field Hospitals: spend a card matching a clearing to move your warriors removed there back to the Keep."],
    ],
  },
  ED: {
    ...
    sections: [
      ...
      ["Birdsong", "Add a card to the Decree. If you have no roost on the map, place a new one."],
      ["Daylight", "Craft, then resolve the Decree left to right (Recruit, Move, Battle, Build). You must resolve every column and cannot end Daylight early."],
      ["Turmoil", "If any Decree card has no legal target you Turmoil: lose 1 VP per bird in the Decree, purge non-vizier cards, depose the leader, choose a new leader, and Daylight ends."],
  ...
  WA: {
    ...
    sections: [
      ...
      ["Birdsong", "Revolt (spend matching supporters to place a base and spark a revolt) or Spread sympathy into an eligible clearing."],
      ["Daylight", "Mobilize (move a supporter to your hand) and Train (spend a matching card to gain an officer)."],
      ["Evening", "Military Operations, one per officer: Organize (place sympathy and score), Move, Battle, Recruit. Then draw cards."],
      ["Rules", "Outrage: whenever another faction removes your sympathy, they must hand you a matching card."],
    ],
  },
  VB: {
    name: "Vagabond",
    ...
    sections: [
      ...
      ["Birdsong", "Refresh: flip up 3 exhausted items, plus 2 per tea on your Refresh track (your choice), then Slip to an adjacent clearing or forest without a boot."],
      ["Daylight", "Exhaust items to act: Move (boot), Battle (sword), Explore (torch), Aid (any item), Quest (two listed items), Strike (crossbow), Repair (hammer), Craft (hammer), Special action (your character)."],
      ["Evening", "Rest (in a forest, repair and refresh damaged items), draw 1 plus 1 per coin, discard to 5, then check your item limit (6 plus 2 per bag)."],
      ["Relationships", "Aid advances a faction's marker one space after 1, 2 and 3 Aids in the same turn. Removing a warrior makes that faction Hostile; you then score Infamy for their pieces."],
    ],
  },
};
